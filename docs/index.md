...
**Health Calculator** — это сервис на Go, который вычисляет агрегированный показатель «здоровья» платформы (`platform_health_score`) на основе метрик из Prometheus. Используется для SLO-мониторинга,合成ного алертинга и визуализации в Grafana.
